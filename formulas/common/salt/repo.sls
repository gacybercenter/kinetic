...
{% if (grains['type'] not in ['cache','salt','pxe'] and salt['mine.get']('role:cache', 'network.ip_addrs', tgt_type='grain')|length != 0) %}
  {% for address in salt['mine.get']('role:cache', 'network.ip_addrs', tgt_type='grain') | dictsort() | random() | last () if salt['network']['ip_in_subnet'](address, pillar['networking']['subnets']['management']) %}
    {% for repo in pillar['cache']['nexusproxy']['repositories'] %}
      {% if grains['type'] == 'arm' %}
        {% if repo['url'] == "https://repo.saltproject.io/salt/py3/ubuntu/22.04/arm64/3006/" + pillar['ubuntu']['name'] %}
    - name: deb http://cache.{{ pillar['haproxy']['sub_zone_name'] }}:{{ pillar['cache']['nexusproxy']['port'] }}/repository/{{ repo }} {{ pillar['ubuntu']['name'] }} main
        {% endif %}
      {% else %}
        {% if repo['url'] == "https://repo.saltproject.io/salt/py3/ubuntu/22.04/amd64/3006/" + pillar['ubuntu']['name'] %}
    - name: deb http://cache.{{ pillar['haproxy']['sub_zone_name'] }}:{{ pillar['cache']['nexusproxy']['port'] }}/repository/{{ repo }} {{ pillar['ubuntu']['name'] }} main
        {% endif %}
      {% endif %}
    {% endfor %}
...
{% else %}
  {% if grains['type'] == 'arm' %}
    - name: deb https://repo.saltproject.io/salt/py3/ubuntu/{{ pillar['ubuntu']['version'] }}/arm64/{{ pillar['salt']['version'] }}/ {{ pillar['ubuntu']['name'] }} main
  {% else %}
    - name: deb https://repo.saltproject.io/salt/py3/ubuntu/{{ pillar['ubuntu']['version'] }}/amd64/{{ pillar['salt']['version'] }}/ {{ pillar['ubuntu']['name'] }} main
  {% endif %}
{% endif %}
