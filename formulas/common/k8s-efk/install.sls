...
grafana_helm_install:
  helm.release_present:
    ...
    - kvflags:
        ...
        set: grafana\.ini.server.domain={{ pillar.get('k8s-efk.grafana.domain') }}
        set: grafana\.ini.server.root_url={{ pillar.get('k8s-efk.grafana.root_url') }}
        set: adminUser={{ pillar.get('k8s-efk.grafana.admin_user') }}
        set: adminPassword={{ pillar.get('opensearch_admin_password') }}
        ...
        set: ingress.enabled={{ pillar.get('k8s-efk.grafana.ingress_enabled') }}
        set: ingress.ingressClassName={{ pillar.get('k8s-efk.grafana.ingress_class') }}
        set: ingress.hosts[0]={{ pillar.get('k8s-efk.grafana.ingress_host') }}
        set: ingress.path=/
