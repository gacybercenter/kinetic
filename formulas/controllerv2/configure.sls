include:
  - formulas/controllerv2/install
  - formulas/common/base
  - formulas/common/networking

{% if 'raid' in pillar['hosts']['controllerv2']['kvm_disk_config']['type'] %}
{% set raid_level = pillar['hosts']['controllerv2']['kvm_disk_config']['type'].split('raid') %}
...
kvm_array:
  raid.present:
    ...
    - level: {{ raid_level[1] }}
    - devices:
    {% for device in pillar['hosts']['controllerv2']['kvm_disk_config']['members'] %}
      - {{ device }}
    {% endfor %}
    - chunk: 512
    - run: true
...
{% elif 'standard' in pillar['hosts']['controllerv2']['kvm_disk_config']['type'] %}
{% set target_device = pillar['hosts']['controllerv2']['kvm_disk_config']['members'][0] %}
