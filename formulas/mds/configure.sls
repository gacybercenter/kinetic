...
make_{{ grains['id'] }}_mdskey:
  cmd.run:
    - name: ceph auth get-or-create mds.{{ grains['id'] }} mon 'profile mds' mgr 'profile mds' mds 'allow *' osd 'allow *' -o /etc/ceph/ceph.mds.{{ grains['id'] }}.keyring
    - creates:
      - /etc/ceph/ceph.mds.{{ grains['id'] }}.keyring
