...
{% set random = salt.cmd.shell("shuf -i 1-"+loop.length|string+" -n 1") %}
{#{% if loop.index != random %}{% continue %}{% endif %}#}
        proxy: http://{{ cache_addresses_dict[host][0] }}:3142 {{ loop.length }} {{ host }} {{ random }} {{ loop.index }}
{% endfor %}
{% endif %}
{% endif %}
{% endfor %}
