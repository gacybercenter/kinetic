...
def application (environ, start_response):
    ...
    if os_assignment == "rocky9":
        ...
        response_body = body % {
            'kernel': f'kernel http://{{ pxe_record }}/tftp/jammy/vmlinuz initrd=initrd url=http://{{ pxe_record }}//tftp/jammy/ubuntu2204.iso autoinstall ip=::::{hostname_assignment}:{interface}:dhcp cloud-config-url=http://{{ pxe_record }}/tftp/assignments/{uuid.upper()}/user-data',
