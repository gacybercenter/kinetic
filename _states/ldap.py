...
def ou_present(name, spec_name, base_dn=None, ous=None):
    ...
    for ou in ous:
        ...
        check_result = __salt__["ldap_utils.dn_exists"](spec_name, base_dn, attributes)
...
def user_present(name, spec_name, base_dn, users=None):
    ...
    for user in users:
        ...
        if not check_result["result"]:
            all_success = False
            comments.append(f"Error checking user {user_dn}: {check_result['comment']}")
            continue
        ...
        if __opts__["test"]:
            ret["result"] = None
            comments.append(f"Would {'update' if exists else 'create'} user {user_dn}.")
